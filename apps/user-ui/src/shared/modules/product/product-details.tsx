...
export default function ProductDetails({
  ...
  const fallbackImage =
    "https://ik.imagekit.io/udemekendrickmurua/murua-logo.png?updatedAt=1777655657397";
  ...
  return (
    <div className="w-full bg-[#f5f5f5] py-5">
        <div className="w-[90%] bg-white lg:w-[80%] mx-auto pt-6 grid grid-cols-1 lg:grid-cols-[28%_44%_28%] gap-6 overflow-hidden">
                    ...
                    <div className="flex gap-2 overflow-auto">
                        {productDetails?.images?.map((img: any, index: number) => (
                            <Image 
                                key={index}
                                src={
                                    img?.url || "https://ik.imagekit.io/udemekendrickmurua/murua-logo.png?updatedAt=1777655657397"
                                }
                                alt="Thumbnail"
                                width={60}
                                height={60}
            ...
            <div className="p-4">
                ...
                <div className="mt-3">
                    <span className="text-3xl font-bold text-orange-500">
                        {(productDetails?.sale_price).toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2, style: "currency", currency: "NGN" })}
                    </span>
                    <div className="flex gap-2 pb-2 text-lg border-b border-b-slate-200">
                        <span className="text-gray-400 line-through">
                            {(productDetails?.regular_price).toLocaleString("en-NG", { minimumFractionDigits: 2, maximumFractionDigits: 2, style: "currency", currency: "NGN" })}
                        </span>
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        {productDetails.sizes.map(
                                            (size: string, index: number) => (
                                                <button
                                                    key={index}
                                                    className={`w-8 h-8 cursor-pointer rounded-full border-2 transition ${isSelected === size ? "border-gray-400 scale-110 shadow-md" : "border-transparent"}`}
            ...
            {/* rigt column - seller information */}
